...
public class ExamineWebsiteTask extends BaseAsyncTask<String, Void, X509Certificate> {
    public ExamineWebsiteTask(Activity activity) {
        super(activity);
    }
    ...
    public X509Certificate doInBackground(String... arg0) {
        try {
            ...
            getActivity().runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    UI.showMessage(ExamineWebsiteTask.this.getActivity(), e);
                }
            });
            return null;
        }
    ...
    public void onPostExecute(X509Certificate result) {
        ...
        if (result != null) {
            CertificateExaminer.examine(getActivity(), result, "Close", new String[0], new DialogInterface.OnClickListener[0]);
        }
    }
}